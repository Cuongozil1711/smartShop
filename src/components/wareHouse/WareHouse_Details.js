...
function WareHouse_Details(props, ref) {
    const { afterSubmit } = props;
    const [show, setShow] = useState(false);
    const refEditMode = useRef(null);
    const dataDefault = {name: "", code: getCode("KHO"), address: ""};
    ...
    const submit = async () => {
        ...
        switch (refEditMode.current) {
            case Enumeration.crud.create:
                WareHouseService.create(itemData).then(
                    (res) => {
            ...
            case  Enumeration.crud.edit:
                WareHouseService.edit(itemData.id ,itemData).then(
                    (res) => {
                        if (afterSubmit && typeof afterSubmit === "function") {
                            setShow(false);
                            afterSubmit(refEditMode.current, res?.data);
    ...
    return (
        ...
        >
            {show && (
                <div>
                    <div className="field">
                        <label htmlFor="name">Mã kho hàng</label>
                    ...
                    <div className="field">
                        ...
                        {submitted && !itemData.name && <small className="p-invalid">Dữ liệu không được để trống</small>}
                    </div>
                    ...
                    <div className="field">
                        ...
                        {submitted && !itemData.address && <small className="p-invalid">Dữ liệu không được để trống</small>}
                    </div>
                </div>
            )}
        </Dialog>
...
WareHouse_Details = forwardRef(WareHouse_Details);
