...
function ReceiptExportWareHouse_Details(props, ref) {
    ...
    const submit = async () => {
        ...
        switch (refEditMode.current) {
            case Enumeration.crud.create:
                ReceiptExportWareHouseService.create(itemData).then(
                    (res) => {
            ...
            case  Enumeration.crud.edit:
                ReceiptExportWareHouseService.edit(itemData.id ,itemData).then(
                    (res) => {
                        if (afterSubmit && typeof afterSubmit === "function") {
                            setShow(false);
                            afterSubmit(refEditMode.current, res?.data);
    ...
    return (
        <Dialog
            visible={show}
            style={{ width: '450px' }}
            header="ReceiptExportWareHouse Details"
            modal
            className="p-fluid"
            footer={() => (
                <>
        ...
        >
            {show && (
                <div>
                    ...
                    <div className="field">
                        <label htmlFor="name" >Tên phiếu xuất kho <span className="text-red-600">*</span></label>
                    ...
                    <div className="field">
                        <label htmlFor="name">Chọn kho xuất hàng <span className="text-red-600">*</span></label>
...
ReceiptExportWareHouse_Details = forwardRef(ReceiptExportWareHouse_Details);

export default ReceiptExportWareHouse_Details;
