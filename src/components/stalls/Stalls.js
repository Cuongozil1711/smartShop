...
import Stalls_Details from "./Stalls_Details";
const Stalls = () => {
    ...
    const loadData = async (_filter, _datas) => {
        try {
            ...
            const res = await StallsService.search(_params);
            if(res?.status == 200){
                const data = res?.data;
                setPosition({
                    ...stalls,
    ...
    const deleteProduct = () => {
        setDeleteItem(false);
        StallsService.delete(itemData?.id).then(
            (res) => {
                loadData({}, stalls);
    ...
    return (
        <div className="grid table-demo">
            <div className="col-12">
                ...
                <div className="card">
                    <div className="col-3">
                        <Button  label={"Thêm quầy hàng"} icon="pi pi-plus" onClick={create} ></Button>
                    </div>
                    <DataTable
                        value={stalls.data}
                        paginator
                        ...
                        <Column
                            ...
                            body={(d) => (
                                <>
                                    ...
                                    <Button icon="pi pi-trash" className="p-button-rounded  mt-2" onClick={() => confirmDeleteProduct(d)} />
                                </>
                            )}
                        ></Column>
                    </DataTable>
            ...
            <Stalls_Details ref={refPosition}  afterSubmit={afterSubmit}/>
            ...
            <Dialog visible={deleteItem} style={{ width: '450px' }} header="Confirm" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                <div className="flex align-items-center justify-content-center">
                    ...
                    {itemData && <span>Bạn có muốn quầy hàng <b>{itemData.name}</b>?</span>}
                </div>
            </Dialog>
        </div>
...
export default React.memo(Stalls, comparisonFn);
